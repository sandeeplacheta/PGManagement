...
export class CommonService  {
  ...
  //   return this.apiSvc.post('/api/master/MasterGetAll', data);
  // }
  getDropDownDataForMaster(masterName: string, primaryKey?: string, select?: string): Observable<any> {
    let queryParams = `mastername=${masterName}`;
    if (primaryKey) {
      queryParams += `&id=${primaryKey}`;
    }
    if (select) {
      queryParams += `&requireString=${select}`;
    }
    return this.apiSvc.get(`/api/master/MasterGetAll?${queryParams}`);
  }
